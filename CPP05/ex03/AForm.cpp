...
std::ostream &operator<<(std::ostream &out, const AForm &form) {
  ...
  std::string boolean_color;

  form.isSigned() ? boolean_color = GREEN : boolean_color = RED;

  out << "AForm:: " << BOLD UNDERLINE << name << RESET << '\n'
      << "\tMinimal signing grade: " << minimal_signing_grade << '\n'
      << "\tMinimal execution grade: " << minimal_exec_grade << '\n'
      << boolean_color << "\tSignature state: " << std::boolalpha
      << form.isSigned() << RESET << '\n'
      << std::endl;
