...
const int &ClapTrap::_getPoints(const PointsType &type, const ClapTrap *clap) {
  switch (type) {
  case (ATTACK_DMG):
    return (clap->_attack_damage);
    break;
  case (ENERGY):
    return (clap->_energy_points);
    break;
  case (HIT):
    return (clap->_hit_points);
    break;
  default:
...
ClapTrap::ClapTrap(const ClapTrap &other)
    : _name(other._getName()), _hit_points(other._getPoints(HIT, this)),
      _energy_points(other._getPoints(ENERGY, this)),
      _attack_damage(other._getPoints(ATTACK_DMG, this)) {
...
ClapTrap &ClapTrap::operator=(const ClapTrap &other) {
  ...
  this->_hit_points = other._getPoints(HIT, this);
  this->_energy_points = other._getPoints(ENERGY, this);
  this->_attack_damage = other._getPoints(ATTACK_DMG, this);

  return (*this);
}

void ClapTrap::attack(const std::string &target) {
  if (_getPoints(ENERGY, this) > 0 && _getPoints(HIT, this)) {
    _message(std::string(_getName() + " attacks " + target + ", causing " +
                         to_string(_getPoints(ATTACK_DMG, this)) +
                         " points of damage!"));

    _decrementPoints(ENERGY);
  } else if (_getPoints(HIT, this) <= 0) {
    _message(std::string(
        _getName() + " is out of hit points and couldn\'t attack " + target));
  } else if (_getPoints(ENERGY, this) <= 0) {
...
void ClapTrap::printState(void) {
  ...
  std::cout << "\tHIT_POINTS: " << _getPoints(HIT, this) << std::endl;
  std::cout << "\tENERGY_POINTS: " << _getPoints(ENERGY, this) << std::endl;
  std::cout << "\tATTACK_DAMAGE: " << _getPoints(ATTACK_DMG, this) << std::endl;
  std::cout << "------" << std::endl;
}
